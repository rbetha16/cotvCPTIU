...
//		oGenericUtils.gfn_Click_On_Object("span", "Return Requires Presentation Rule");
	...
	@Step
	public void ruleReviewErrorMsg(String arg1) throws InterruptedException {
		...
		clickInstanceinAdmin("FINAL MD DEL");
		oGenericUtils.gfn_Click_String_object_Xpath("//div[@class='GEFT4QHBFP GEFT4QHBKP']");
		int iRuleReviewRequest = getDriver().findElements(By.xpath("//span[text()='Rule Review Request']")).size();
		if(iRuleReviewRequest>0) {
			oSeleniumUtils.Click_given_Locator(StringUtil.replace(oIUHomePage.SpanTag, "sValue", "Rule Review Request"));
		}else {
			oSeleniumUtils.Click_given_Locator(StringUtil.replace(oIUHomePage.DivTag, "sValue", "Rule Review Request"));	
...
//		oSeleniumUtils.Click_given_Locator(StringUtil.replace(oIUHomePage.DivTag, "sValue", "Rule Review Request"));
		oSeleniumUtils.Enter_given_Text_Element("//label[text()='Enter rule for proposal assessment:']/..//div//div//div//input", Serenity.sessionVariableCalled("MidRuleVersion").toString());
		oSeleniumUtils.Click_given_Locator(StringUtil.replace(oIUHomePage.DivTag, "sValue", "Request Rule Review"));
		
		String sMidRule = Serenity.sessionVariableCalled("MidRuleVersion").toString();
		String sText =oSeleniumUtils.get_StringTextFrom_Locator("//div[contains(text(),'The selected rule')]");
		System.out.println(sText);
		String sExpectedText ="The selected rule ("+sMidRule+") is already part of the current update instance:";
		System.out.println(sExpectedText);
		
		int iText = getDriver().findElements(By.xpath("//div[contains(text(),'The selected rule ("+sMidRule+") is already part of the current update instance:')]")).size();
		
		if (iText>0) {
			verify("The selected rule....Messege is dispalyed", true);
		}else {
			verify("The selected rule....Messege is dispalyed", false);
		}
		...
		// Serenity.setSessionVariable("MidRuleV.ersion").to("2951.162");
